export function buildQueryOptions(modelConfig, query = {}, userId) {
  const { searchableFields = [], filterableFields = [], relations = {}, hasSoftDelete = true } = modelConfig;

  const { get_all = false, pagination, order_by, include_relation = [], search, filter = {} } = query;
  ...
  // 📊 Order By
  const orderBy = Array.isArray(order_by) ? order_by.map(({ field, direction }) => ({ [field]: direction })) : [];
