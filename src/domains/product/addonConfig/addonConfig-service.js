...
class AddonConfigService {
	...
	async findById(id, userId) {
		...
		if (addonConfig.owned_by !== userId) {
			throw BaseError.forbidden(
				"You are not allowed to access this add-on config.",
			);
		}
	...
	async create(data) {
		...
		if (inventoryExists.length !== inventoryIds.length)
			throw BaseError.notFound(
				"Duplicate inventory IDs or some do not belong to the user.",
			);
		...
		if (existingConfigs.length > 0) {
			...
			throw BaseError.badRequest(
				`Configuration already exists for config(s): ${existingIds.join(", ")}`,
			);
		}
