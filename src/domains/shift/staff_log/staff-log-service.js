...
class StaffLogService {
  async getStaffLogs(userId, query) {
    ...
    if (query?.advSearch) {
      if (query?.advSearch?.shift_date) {
        ...
        const startOfDay = new Date(shiftDate.getFullYear(), shiftDate.getMonth(), shiftDate.getDate(), 0, 0, 0);
        const endOfDay = new Date(shiftDate.getFullYear(), shiftDate.getMonth(), shiftDate.getDate(), 23, 59, 59);
        options.where.AND = [
          ...(options.where.AND || []),
          {
            start_timestamp: {
  ...
  async getStaffLogById(id, userId) {
    let log = await db.staff_log.findUnique({
      where: { id },
      include: {
        staff: true,
      },
