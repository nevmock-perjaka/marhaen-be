...
class StockService {
	async reduceStockByOrder(orderId) {
		...
		const existingLog = await db.stock_log.findUnique({
			where: { order_id: orderId },
		});
		if (existingLog)
			throw BaseError.badRequest(
				"Stok dari order ini sudah dikurangi sebelumnya.",
			);
	...
	async addStockFromInput(inputHistoryId) {
		...
		const existingLog = await db.stock_log.findUnique({
			where: { input_id: inputHistoryId },
		});
		if (existingLog)
			throw BaseError.badRequest(
				"Stok dari input ini sudah ditambahkan sebelumnya.",
			);
	...
	async _adjustStock(inventoryId, delta) {
		...
		if (!latest)
			throw BaseError.notFound(
				"Input history untuk inventory tidak ditemukan.",
			);
