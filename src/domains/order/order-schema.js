...
const orderSchema = {
  ...
  params: Joi.object({
    ...
    order_by: Joi.array()
      .items(
        Joi.object({
          field: Joi.string().valid("order_by", "phone_number", "status", "total_gross", "created_at").required(),
    ...
    filter: Joi.object({
      ...
      .custom((value, helpers) => {
        ...
        if (conflict.length > 0) {
          return helpers.message(`Field(s) [${conflict.join(", ")}] cannot be in both 'is_null' and 'is_not_null'`);
        }
