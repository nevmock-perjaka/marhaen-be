...
class OrderService {
  ...
  async create(data, strict_mode = false) {
    return db.$transaction(async (tx) => {
      ...
      if (!midtransConfig.server_key || !midtransConfig.client_key)
        throw BaseError.badRequest("Midtrans server key or client key is not set.");
      ...
      if (data.table_id && !tableExists.is_active)
        throw BaseError.badRequest("Table is not active. Please activate the table first.");
      ...
      if (data.shareable_code && !discountExists.is_active)
        throw BaseError.badRequest("Discount is not active. Please activate the discount first.");
      ...
      if (data.shareable_code && discountExists.used >= discountExists.max_use)
        throw BaseError.badRequest("Discount has reached its maximum usage limit.");
      ...
      if (productExists.length !== productIds.length)
        throw BaseError.badRequest("Some products do not exist or are not owned by the user.");

      // Check if the add-ons exist and are owned by the user
      const addOnIds = [...new Set(data.order_items.flatMap((item) => item.order_item_add_ons || []))];
      ...
      if (addOns.length !== addOnIds.length)
        throw BaseError.badRequest("Some add-ons do not exist or are not owned by the user.");

      // Mapping add_on_id → product_id yang seharusnya
      const addOnToProductMap = new Map(addOns.map((addOn) => [addOn.id, addOn.add_on_group.product_id]));
      ...
      if (!snap)
        throw BaseError.serviceUnavailable("Midtrans service is currently unavailable. Please try again later.");
  ...
  async updateWebhookMidtrans(data) {
    ...
    const hash = crypto
      .createHash("sha512")
      .update(`${data.order_id}${data.status_code}${data.gross_amount}${decrypt(user_midtrans.server_key)}`)
      .digest("hex");
    ...
    if (orderTransaction.owned_by !== user.id)
      throw BaseError.forbidden("You are not allowed to access this order transaction.");
