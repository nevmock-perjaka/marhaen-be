...
class ProfileService {
	...
	async login(user_id, profile_id, pin) {
		...
		if (!profile.pin && pin)
			throw BaseError.badRequest("Profile does not have a PIN set");
	...
	async updateProfilePin(userId, data) {
		...
		if (profile.user_id !== userId)
			throw BaseError.forbidden("You are not allowed to access this profile.");

		// Validasi jika PIN sudah diset di DB
		if (profile.pin) {
			if (!data.old_pin)
				throw BaseError.badRequest("Old PIN is required to update PIN");
			if (profile.pin !== data.old_pin)
				throw BaseError.badRequest("Old PIN is incorrect");
		} else {
